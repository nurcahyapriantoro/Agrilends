    ...
    match call_result {
        Ok((Ok(block_index),)) => {
            ...
            let disbursement = DisbursementRecord {
                ...
                disbursed_at: ic_cdk::api::time(),
                disbursed_by: ic_cdk::caller(),
            };
...
pub async fn process_ckbtc_repayment(
    ...
    let transfer_args = TransferArgs {
        ...
        to: Account {
            owner: ic_cdk::id(), // Transfer to this canister
            subaccount: None,
        },
        amount: Nat::from(amount),
        fee: None,
...
pub async fn check_ckbtc_balance(account: Account) -> Result<u64, String> {
    ...
    let call_result: CallResult<(Nat,)> = 
        ic_cdk::call(ckbtc_ledger, "icrc1_balance_of", (balance_args,)).await;
...
fn calculate_remaining_balance(loan_id: u64) -> Result<u64, String> {
    ...
    let elapsed_time = ic_cdk::api::time() - loan.created_at;
